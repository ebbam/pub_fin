...
py_test <- readRDS(here("data/temp/county_py_test.rds"))
#test <- read.csv(here("data/temp/county_py_test.csv"))

#temper %>% write.csv(here("data/temp/temper_r_test.csv"))

#test %>% as.data.frame %>% select(c("year", "county_name", "item_formal", "amount")) #%>% all.equal(select(temper, c("year", "county_name", "item_formal", "amount"))
