...
Preliminary model results making use of shift-share instruments in employment and wage changes coming from coal, extraction, and oil & gas industries. 
...
The first section uses overall public expenditure as a dependent variable. The second section breaks this expenditure down into the four categories mentioned above. 

*Next steps:*          
- Reconsider control variables beyond county-level personal income.      
- Replace personal income with GDP now that time series is shortened as a consequence of data availability in the shift-share instrument).        
- Experiment with different lags of the independent variable (currently assumes same-period change).      
- Experiment with different base years for the shift-share instrument (currently constructed from base year 2001).   
...
shift_shares <- readRDS(here('data/temp/shift_shares_2001_base.RDS')) %>% 
  mutate(fips = ifelse(fips == "15009", "15901", fips),
         fd_total_annual_wages_2121_k = fd_total_annual_wages_2121/1000,
         ss_wage_coal_k = ss_wage_coal1000)
...
test <- feols(data = tots_df_ss, as.formula("amount_pc ~ -1 + pinc_k_pc | fips + year | fd_total_annual_wages_2121_k ~ ss_wage_coal_k"), panel.id = c('year', 'fips'), se = "twoway")

test %>% etable
              
              

iv_forms <- paste0(" ~ -1 + pinc_k_pc | fips + year ", c(" | fd_total_annual_wages_2121 ~ ss_wage_coal",
                     "| fd_total_annual_wages_21 ~ ss_wage_extraction",
                     "| fd_total_annual_wages_211 ~ ss_wage_oil_gas",
                     "| fd_annual_avg_emplvl_2121 ~ ss_emp_coal",
                    "| fd_annual_avg_emplvl_21 ~ ss_emp_extraction", 
                    "| fd_annual_avg_emplvl_211 ~ ss_emp_oil_gas"))

names(iv_forms) <- c("wage_coal", "wage_extraction", "wage_oil_gas", "emp_coal", "emp_extraction", "emp_oil_gas")
#ss_emp_ff ~ fd_annual_avg_emplvl_XX, 
#ss_wage_ff ~ fd_annual_wages_XX, 


mods_pub_exp_total <- list()
for(form in iv_forms){
    forms <- paste0("amount_pc", form)
    mods_pub_exp_total[[paste0(names(iv_forms[iv_forms == form]))]] <- feols(data = tots_df_ss, as.formula(forms), panel.id = c('year', 'fips'), se = "twoway")
  }

#setFixest_etable(markdown = TRUE, page.width = "a4") #style.tex = my_style, 

print("TOTAL PUBLIC EXPENDITURE (PC)")
mods_pub_exp_total %>% etable(order = "!Personal")
...
for(dep in deps[deps != "pinc_k_pc*1000"]){
  temp_list <- list()
  nam <- paste0("mods_", str_sub(dep, end=-6))
  for(form in iv_forms){
    forms <- paste0(dep, form)
    temp_list[[paste0(names(iv_forms[iv_forms == form]),"_", str_sub(dep, end=-6))]] <- feols(data = broad_df_ss, as.formula(forms), panel.id = c('year', 'fips'), se = "twoway")
  }
  assign(nam, temp_list)
}

print("ADMINISTRATION & MISC EXPENDITURE (PC)")
mods_administration_misc_pc %>% etable(order = "!Personal")

print("COMMERCIAL EXPENDITURE (PC)")
mods_commercial_pc %>% etable(order = "!Personal")

print("INFRASTRUCTURE, UTILITIES & REGULATION (PC)")
mods_infrastructure_utilities_regulation_pc %>% etable(order = "!Personal")

print("PUBLIC WELFARE, SERVICES & FACILITIES (PC)")
mods_public_welfare_services_facilities_pc %>% etable(order = "!Personal")
