...
dfi <- readRDS(here("data/temp/pubexp_1970_2020_controls.RDS"))
...
tots <- readRDS(here("data/temp/pubexp_1970_2020_totals_controls.RDS")) %>% 
...
# Data Coverage
## Geography - Broad Categories and Total Expenditure

The animations below show the data coverage at a county level of the 4 broad categories (purple) and total expenditure values (green). Coverage is strong at this level. (Apologies for the poor arrangement!)

```{r, echo = FALSE}

plot_list <- paste0(here("output/mark/"), list.files(here("output/mark/")))

knitr::include_graphics(plot_list)
...
# Histogram of missing values overall
temp_dfi %>% 
  group_by(broad_cat) %>% 
  mutate(n_sum_cat = n_distinct(sum_category)) %>% 
  ungroup %>% 
  filter(is.na(amount)) %>% 
  ggplot() + 
  geom_histogram(aes(x = year, fill = broad_cat), bins = 51) +
  geom_hline(aes(yintercept = n_sum_cat * n_distinct(temp_dfi$fips))) +
...
temp_dfi %>% 
  ...
  filter(is.na(amount)) %>% 
  ggplot() + 
  geom_histogram(aes(x = year, fill = sum_category), bins = 51) +
  #geom_hline(aes(yintercept = n_fips * n_distinct(temp_dfi$sum_category))) + 
  facet_wrap(~broad_cat, scales = "free")
...
temp_dfi %>% 
  filter(is.na(amount)) %>% 
  ggplot() + 
  geom_histogram(aes(x = year, fill = state), bins = 51) +
  geom_hline(aes(yintercept = n_distinct(temp_dfi$fips) * n_distinct(temp_dfi$sum_category)))
...
*Dependent variables*: Expenditure by category in levels and per capita values. \
...
broad_df <- temp_dfi %>% 
  ...
  complete(fips, year, broad_cat) %>% 
  rename(tot = amount, pc = amount_pc) %>%
  # START HERE: NEED TO MAKE SURE THAT ALL CONTROL VALUES ARE RETAINED
  pivot_wider(id_cols = c(fips, year), names_from = broad_cat, values_from = c(tot, pc), names_glue = "{broad_cat}_{.value}") %>% 
  clean_names %>% arrange(fips, year)
  
#identical(broad_df, broad_df2)
...
time_broad_raw <- create_res(df = broad_df, cats = broad_cats[grepl("tot", broad_cats)], regx = " ~ year | fips", suffix = "_tot")
time_broad_inter <- create_res(df = broad_df_inter, cats = broad_cats[grepl("tot", broad_cats)], regx = " ~ year | fips", suffix = "_tot")

# Time trends: PC expenditure with and without interpolated data
time_broad_raw_pc <- create_res(df = broad_df, cats = broad_cats[grepl("pc", broad_cats)], regx = " ~ year | fips", suffix = "_pc")
time_broad_inter_pc <- create_res(df = broad_df_inter, cats = broad_cats[grepl("pc", broad_cats)], regx = " ~ year | fips", suffix = "_pc")


tk <- full_join(time_broad_raw$tbl,
          time_broad_inter$tbl, by = c("outcome_var", "_"), suffix = c(".time_raw", "")) %>% 
  full_join(., time_broad_raw_pc$tbl, by = c("outcome_var", "_"), suffix = c(".time_inter", "")) %>% 
          full_join(., time_broad_inter_pc$tbl, by = c("outcome_var", "_"), suffix = c(".time_raw_pc", ".time_inter_pc")) %>% 
  select(-`_`) %>% 
...
broad_plms <- feols(data = broad_df, .[deps]  ~ pinc_pc | fips + year, panel.id = c('year', 'fips'), se = "twoway")
broad_plms_inter <- feols(data = broad_df_inter, .[deps]  ~ pinc_pc | fips + year, panel.id = c('year', 'fips'), se = "twoway")


# Total expenditure
tot_mod_pc <- feols(data = tots, amount_pc ~ pinc_pc | fips + year, panel.id = c('year', 'fips'), se = "twoway")
tot_inter_pc <- create_res(df = inter_tots, amount_pc ~ pinc_pc | fips + year, panel.id = c('year', 'fips'), se = "twoway")
