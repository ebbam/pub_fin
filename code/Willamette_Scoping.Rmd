...
#old_diffs <- county_cats %>% filter(!grepl("Total", large_category)) %>% group_by(fips, year) %>% summarise(sum #= sum(amount)) %>% ungroup %>% left_join(., county_total, by = c("fips", "year")) %>% mutate(test_diff = amount #- sum)

county_short <- readRDS(here("data/temp/county_short_expenditure.RDS"))

county_total <- county_short %>% 
  select(fips, year, county_name, fips_state, Population, item_formal, item, amount) %>% 
  filter(item_formal == "Total_Expenditure") %>% 
  # Testing to see if year-fips observations are unique: Check!
  # county_total %>% select(year, fips) %>% distinct %>% nrow %>% identical(nrow(county_total))
...
county_cats %>% filter(!grepl("Total", large_category)) %>% group_by(fips, year) %>% summarise(sum = sum(amount)) %>% left_join(., county_total, by = c("fips", "year")) %>% mutate(test_diff = amount - sum) %>% arrange(test_diff) # %>% ungroup %>% identical(old_diffs)

county_cats %>% filter(!grepl("Total", large_category)) %>% group_by(fips, year) %>% summarise(sum = sum(amount)) %>% left_join(., county_total, by = c("fips", "year")) %>% mutate(test_diff = amount - sum) %>% ggplot(aes(x = test_diff)) + geom_histogram(bins = 10) + geom_vline(xintercept = 0, linetype = "dashed", color = "red") + facet_wrap(~year)
...
# Shows coverage (binary) of our categories across our time period 2000-2020
county_rel %>% 
  mutate(amount = ifelse(is.na(amount), 0, amount),
         present = ifelse(amount != 0, 1, 0)) %>% 
...
# Shows coverage (magnitude) of our categories across our time period 2000-2020
county_rel %>% 
  group_by(year, sum_category)  %>% 
