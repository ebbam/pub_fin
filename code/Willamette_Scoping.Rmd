...
pop_70_09 %>% 
  ...
  # Check to see whether any fips values are unmatched
   filter(is.na(pop) & year <= 2009) %>% pull(fips) %>% unique



# Checking Virginia coverage for 51123 fips code issue....
# temp %>% 
...
#   scale_fill_continuous(trans = 'reverse')
# 
# # Checking completeness of GDP data
# ## Confirms all FIPS codes in full dataset are present in GDP dataset
# temp %>% 
#   pull(fips) %>% unique %>% setdiff(., unique(temp_gdp$fips)) %>% length(.) == 0
# 
# incomp_pop <- temp_pop %>% 
#   filter(fips %in% temp$fips) %>% 
#   filter(!complete.cases(.)) %>% pull(fips) %>% 
#   unique
# 
# temp %>% filter(fips %in% incomp_pop) %>% select(fips, year) %>% distinct %>% 
#   mutate(present = 1) %>% 
#   #pivot_wider(id_cols = fips, names_from = year, values_from = present) %>% 
#     ggplot(aes(x = year, y = fips, fill = present)) +
#   geom_tile()
# 
# 
# pop_70_09 %>% 
#   filter(fips %in% incomp_pop) %>% 
#   select(-c(state_fips, county_fips, areaname, state_name, county_name, fipsst, fipsco, region, division)) %>% 
#   pivot_longer(cols = starts_with("pop"), names_to = "year", values_to = "pop", names_prefix = "pop") %>% 
#   #pivot_wider(id_cols = fips, names_from = year, values_from = present) %>% 
#   ggplot(aes(x = year, y = fips, fill = pop)) +
#   geom_tile()
# 
# # Final pivoted GDP file


```

# County Intercensal population data: 2010-2020:
...
# This dataset includes additional control variables related to birth rates, deathhs, international and domestic migration rates
